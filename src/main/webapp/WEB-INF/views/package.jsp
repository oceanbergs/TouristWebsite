...
.image-container {
    ...
    display: flex; /* Ensure proper centering (if needed) */
    justify-content: center; /* Center image horizontally */
    align-items: center; /* Center image vertically */
    background-color: #f9f9f9; /* Optional background for a polished look */
}
...
.fixed-size-image {
    ...
    object-position: center; /* Align the image content to the center */
}
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // Attach click event to the Book Now button
        document.getElementById('bookNowBtn').addEventListener('click', function () {
            // Get input values
            const name = document.getElementById('name').value.trim();
            const email = document.getElementById('email').value.trim();
            const mobile = document.getElementById('mobile').value.trim();
            const destination = document.getElementById('destinationDropdown').value.trim();
            const startDate = document.getElementById('startDate').value;
            const endDate = document.getElementById('endDate').value;
            const message = document.getElementById('message').value.trim();

            // Validate inputs
            if (!name || !email || !mobile || !destination || !startDate || !endDate) {
